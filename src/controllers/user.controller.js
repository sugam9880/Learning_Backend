...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    // console.log(
    //   "user insdie generateAccessAndRefreshToken => getting from DB",
    //   user
    // ); ////////////////
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!avatar)
    throw new ApiError(
      400,
      "Avatar Image Is not uploading in cloudinary and it is a compulsray field"
    );
  ...
  // waiting
  const userCreated_ = await User.findById(userDB._id).select(
    // it is a DB query where we get data from DB
    "-password -refreshToken"
  );

  console.log(
    "getting data from database but expect password and refreshtoken",
    userCreated_
  ); ///////////////
  ...
  return res
    .status(201)
    .json(new apiResponse(200, userCreated_, "user registered successfully"));
});
...
const getCurrUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(new apiResponse(200, {}, "current user got successfully"));
});
...
const updataUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImageLocalPath) {
    throw new ApiError(
      400,
      "coverImageLocalPath local file not found or it doesnot exist"
    );
  }
